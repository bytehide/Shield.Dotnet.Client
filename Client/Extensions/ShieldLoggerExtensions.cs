...
namespace Shield.Client.Extensions
{
    public static class ShieldLoggerExtensions
    {
        ...
        public static void OnSuccess(this ProtectionResult result, ServerSentEvents connection, Action<ProtectedApplicationDto> action)
            => connection.On(result.OnSuccess, (message, level, time) => action(JsonSerializer.Deserialize<ProtectedApplicationDto>(message)));
        public static void OnError(this ProtectionResult result, ServerSentEvents connection, Action<string> action)
            => connection.On(result.OnError, (message, level, time) => action(message));
        public static void OnClose(this ProtectionResult result, ServerSentEvents connection, Action<string> action)
            => connection.On(result.OnClose, (message, level, time) => action(message));
